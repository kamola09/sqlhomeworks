...
select count(CustomerID) as CountCustomer from  Customers
...
select CustomerID, Sum(SaleAmount) as totalCustomer from Sales
group by CustomerID;
--10
Select * from Employees
select DepartmentName, count(EmployeeID) as CountEmploye from Employees
...
select Category, sum(SaleAmount) as sumSale , avg(SaleAmount) as avgSale from Sales s

join Products P on s.ProductID = P.ProductID
group by Category;
--12
select * from Employees
select  count(DepartmentName) as countemp from Employees
where DepartmentName ='HR';

--13
select DepartmentName, max(salary)as maxSalary,min(salary)  MinSalary from Employees
group by DepartmentName;
--14
select DepartmentName,avg(Salary) as avgSalary  from Employees
group by DepartmentName;
--15
select DepartmentName, count(EmployeeID) as countEmployeId,avg(salary) as avgSalary from Employees
group by DepartmentName;
--16
...
--17
select * from Sales
select YEAR(SaleDate), sum(SaleAmount) as SaleYEar from Sales
...
select CustomerID,sum(SaleAmount) as totalPrice from Sales
group by CustomerID
having sum(SaleAmount) > 1500;
 --22
 select DepartmentName,avg(salary) as avgSalary, sum(salary) totalsalary  from Employees
group by DepartmentName
having  avg(salary) > 65000 and sum(salary) > 65000;

--23
select * from Sales
DECLARE @usdRate DECIMAL(10,2) = 12500; -- 1 dollarning kursi
DECLARE @amountUSD DECIMAL(10,2) = 50;  -- berilgan dollar miqdori
DECLARE @amountUZS DECIMAL(18,2) = @usdRate * @amountUSD;
SELECT CustomerID,
       SUM(CASE WHEN @amountUZS> 50000 THEN SaleAmount ELSE 0 END) AS TotalExpensiveOrders,
...
select  year(OrderDate),month(OrderDate), sum( P.Price) as total, count(distinct P.ProductID)as uniqueProducts  from Orders O join Products P on P.ProductID=O.ProductID
group by YEar(OrderDate),month(O.orderDate)
having count( distinct P.ProductID) > 2;
--25
select * from Orders
select Year(OrderDate) as yearorder ,min(Quantity) as minquantity,max(Quantity) as MaxQuantity from Orders
